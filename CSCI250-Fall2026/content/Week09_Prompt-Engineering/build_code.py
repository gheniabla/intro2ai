...
reasoning = [
    ...
    ("md", "## 7. Extended thinking / test-time compute (2026)\n"
           "CoT is *you* asking for steps. **Extended thinking** lets the model spend a "
           "**budget** of hidden reasoning tokens *before* it answers — the \"think longer\" "
           "knob. **Claude** takes `thinking={'type':'enabled','budget_tokens':N}`; "
           "**Gemini 2.5** has a `thinking` mode. Great for hard, multi-step problems; on "
           "easy ones it just costs more (\"overthinking\"). Open reasoning models like "
           "**DeepSeek-R1** do this by default (run locally via Ollama). Runs offline below "
           "via a scripted fallback."),
    ("code", "def think_claude(prompt, budget_tokens=1024, max_tokens=2000):\n"
    ...
    ("md", "---\n### Takeaways\n"
           ...
           "- **Extended thinking / test-time compute** lets the model think longer via a "
           "token **budget** — worth it for hard problems, wasteful (\"overthinking\") on easy ones.\n"
