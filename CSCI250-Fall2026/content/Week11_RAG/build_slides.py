...
slides = [
    ...
    {"type": "bullets", "title": "Cite Sources & Know Your Limits", "bullets": [
        ...
        ("Responsible AI: RAG can still hallucinate — cite sources; "
         "retrieval amplifies corpus bias", 1)]},

    {"type": "section", "title": "Modern RAG (2026) — Beyond Plain Vector Search"},
    {"type": "two_col", "title": "Four Upgrades on Top of Dense + Fixed Chunks",
     "left_title": "Retrieve better",
     "left": ["Hybrid search: dense + BM25 keyword, FUSED",
              "  -> catches paraphrases AND exact tokens",
              "Rerank: cross-encoder reorders the top-k",
              "  -> reads (question, chunk) together"],
     "right_title": "Smarter context & control",
     "right": ["Contextual Retrieval (Anthropic):",
               "  prepend a context blurb before embedding",
               "Agentic RAG: model decides when/what",
               "  to retrieve; multi-step lookups",
               "Add only where the triad says it's the bottleneck"]},
    ...
    {"type": "closing", "title": "This Week — To Do", "bullets": [
        ...
        "Run 03_rag_evaluation.ipynb (RAG triad + re-measure; key for real numbers)",
        "Optional: 04_hybrid_rerank.ipynb (hybrid + cross-encoder rerank)",
